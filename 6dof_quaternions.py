...
pf_frame = np.eye(4)
pf_frame[:3, :3] = ikpy.geometry_utils.rpy_matrix(0, 0, 0)
...
K_o = 2
K_p = 2 * np.eye(3)
t_d, dt_d, r, simulation_time = tg.trajectory_generation(dt, tf, 0.1 * tf, p0_frame, pf_frame)
# print(p_d)
...
for i in range(0, len(simulation_time) - 1):
    ...
    # print(omega_e.shape)
    ita_e, e_e = ikpy.geometry_utils.quaternion_from_rotation_matrix(p_star_frame[:3, :3])
    ita_d, e_d = ikpy.geometry_utils.quaternion_from_rotation_matrix(np.dot(p0_frame[:3, :3], ikpy.geometry_utils.rotation_matrix_from_angle_axis(t_d[3, i], *r)))
    ...
    # thetadot = get_angle_difference(a_star, b_star, c_star, p_d[3, i], p_d[4, i], p_d[5, i]) * 0.5
    # print(pdot, thetadot)
