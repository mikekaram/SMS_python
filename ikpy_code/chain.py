...
class Chain(object):
    ...
    def symbolic_transformation_matrix(self, link_number):
        ...
        symbolic_transformation_matrix = sp.eye(4)
        # print(symbolic_transformation_matrix[1])
        Ja = sp.zeros(3, 1)
        ...
        # print("Shape of Symbolic Transformation Matrix is:", symbolic_transformation_matrix.shape)
        return symbolic_transformation_matrix

    def jacobian_matrix(self, sym_mat):
        theta0, theta1, theta2, theta3, theta4, theta5 = sp.symbols('theta0 theta1 theta2 theta3 theta4 theta5')
        theta = sp.Matrix([theta0, theta1, theta2, theta3, theta4, theta5])
        Jl = sp.zeros(3, 1)
        for index, link in enumerate(self.links[1:]):
            # print("Index is:", index)
            # print("dpx/dq with theta%d:" % index, sym_mat[3].diff(theta[index]))
            # print("dpy/dq with theta%d:" % index, sym_mat[7].diff(theta[index]))
            # print("dpz/dq with theta%d:" % index, sym_mat[11].diff(theta[index]))
            Jl = Jl.col_insert(index + 1, sp.Matrix([sym_mat[3].diff(theta[index]), sym_mat[7].diff(theta[index]), sym_mat[11].diff(theta[index])]))
        #     print("Jl after theta%d:" % index, Jl)
        # print("Jl shape:", Jl.shape)
        # print(Jl[0])
        # print(Jl[7])
        # print(Jl[14])
        Jl.col_del(0)
        # print("Final Jl is:", Jl)
        # print(self.Ja)
        J = sp.Matrix([Jl, self.Ja])
        return J
